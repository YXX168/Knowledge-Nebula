# 知识星云（Knowledge Nebula）

> 把本机文件夹变成可搜索、可预览、可安全编辑的个人知识库。

知识星云是一款本地优先、自托管的知识库工作台。它直接读取你指定的文件夹，提供全文搜索、多格式预览、在线编辑和文件变更同步；知识内容始终保留在自己的设备或服务器上，不依赖第三方云服务。
...
- 当前版本不包含用户鉴权。不要在没有 HTTPS、身份验证和访问控制的情况下直接暴露到公网。

发现安全问题时，请不要创建公开 Issue，具体报告方式见 [安全策略](SECURITY.md)。
...
问题反馈和代码贡献请参阅 [贡献指南](CONTRIBUTING.md)。
